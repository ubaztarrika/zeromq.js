...
var Socket =
exports.Socket = function (type, exposeMsgBufferClearApi = false) {
  var self = this;
  ...
  this._exposeMsgBufferClearApi = exposeMsgBufferClearApi;
...
Socket.prototype._emitMessage = function (message) {
  if (message.length === 1) {
    // hot path
    if(this._exposeMsgBufferClearApi){
      this.emit('message', {...message[0], clear: () => this._clearMsgBuffer(message[0].id, message[0].msgref)});
    } else {
    ...
    if(this._exposeMsgBufferClearApi){
      this.emit.apply(this, ['message'].concat({...message, clear: () => this._clearMsgBuffer(message.id, message.msgref)}));
    } else {
...
Socket.prototype._flushRead = function () {
  try {
    var message = this._zmq.readv(this._exposeMsgBufferClearApi); // can throw
    if (!message) {
      return false;
    }
...
Socket.prototype._clearMsgBuffer = function (id, msgref) {
  try {
    /*var message = */this._zmq.clearMsgBuffer(id, msgref); // can throw
    /*if (!message) {
      return false;
    }*/
    // Handle received message immediately to prevent memory leak in driver
    //this._emitMessage({...message, clear: () => this._clearMsgBuffer(message.id, message.msgref)})
  } catch (error) {
    this.emit('error', error); // can throw
  }
  return true;
...
Socket.prototype._flushReads = function(enableImmediateGc) {
  if (this._paused || this._isFlushingReads) return;

  this._isFlushingReads = true;

  while (this._flushRead(enableImmediateGc));
...
exports.socket =
exports.createSocket = function(type, exposeMsgBufferClearApi, options) {
  var sock = new Socket(type, exposeMsgBufferClearApi);
